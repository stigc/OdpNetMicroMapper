...
namespace TestManagedCore
{
    ...
    public class Test : TestBase
    {
        ...
        public void InsertStatic()
        {
            ...
            orm.Insert(item, "onmm3.item");

            item = orm.Query<Item>("select * from onmm3.item where id = :0", 3).SingleOrDefault();
        ...
        {
            var guid = new Guid();
            orm.NonQuery("insert into onmm3.rawtest (bytes) values (:0)", guid.ToByteArray());

            var bytesLoaded = orm.QueryScalar<byte[]>("select bytes from onmm3.rawtest where rownum = 1");
            var guidLoaded = new Guid(bytesLoaded);
            Assert.AreEqual(guid, guidLoaded);

            dynamic item = orm.Query("select bytes from onmm3.rawtest where rownum = 1").Single();
        ...
        {
            dynamic item = new Entity("onmm3.item");
            item.Id = 3;
            item.Name = "Third Item";
            ...
            item = orm.Query("select * from onmm3.item where id = :0", 3).Single();
            Assert.AreEqual(3, item.Id);
        ...
        {
            dynamic item = new Entity("onmm3.item", "id");
            item.Id = 1;
            item.Name = null;

            orm.Update(item);

            item = orm.Query("select * from onmm3.item where id = :0", 1).Single();
        ...
        {
            dynamic item = new Entity("onmm3.item_with_long");
            item.Text = new string('X', 1000 * 4); //max 4k when inserting!
            orm.Insert(item);

            var items = orm.Query("select * from onmm3.item_with_long");
        ...
        {
            dynamic item = new Entity("onmm3.item");
            Assert.AreEqual(2, orm.QueryScalar<int>("select count(1) from onmm3.item"));

            orm.Delete(item, "where id = :0", 1);
            Assert.AreEqual(1, orm.QueryScalar<int>("select count(1) from onmm3.item"));

            orm.Delete(item, "where id = :0", 2);
            Assert.AreEqual(0, orm.QueryScalar<int>("select count(1) from onmm3.item"));
        }
        ...
        public void DeleteWithKeyMetadata()
        {
            ...
            var items = orm.Query("select * from onmm3.item");
        ...
        {
            Assert.AreEqual(null, orm.QueryScalar<int?>("select 1 from onmm3.item where 1=2"));
        }
        ...
        public void UpdateViaWhereClause()
        {
            ...
            var items = orm.Query("select id, name from onmm3.item order by id");
        ...
        public void MergeInto()
        {
            ...
            var itemLoaded = orm.Query<Item>("select * from onmm3.item where id = :0", 100).SingleOrDefault();
            Assert.AreEqual(item.Name, itemLoaded.Name);

            item.Name = "renamed";
            orm.MergeInto(item);

            itemLoaded = orm.Query<Item>("select * from onmm3.item where id = :0", 100).SingleOrDefault();
            Assert.AreEqual(item.Name, itemLoaded.Name);
        }
        ...
        {
            Entity item = orm.Query("select yield_2date from onmm3.item_odd where id = 1").Single();
        ...
        {
            var item = orm.Query("select * from onmm3.item where id = 1").Single();
        ...
        {
            var ints = orm.QuerySingleTypeList<int>("select id from onmm3.item order by id").ToList();
            Assert.AreEqual(1, ints[0]);
            Assert.AreEqual(2, ints[1]);

            var strings = orm.QuerySingleTypeList<string>("select name from onmm3.item order by id").ToList();
        ...
        {
            var items = orm.Query<Item>("select * from onmm3.item order by id").ToList();
        ...
        {
            var item = orm.Query<Item>("select id, name from onmm3.item where id = :0", 1)
                .SingleOrDefault();
            Assert.AreEqual(1, item.Id);
            Assert.AreEqual("First Item", item.Name);
        }
        ...
        {
            var result = orm.ExecuteFunction<int>("onmm3.plus1function", 100);
        ...
        {
            var result = orm.ExecuteFunction<string>("onmm3.append1function", "100");
        ...
        {
            var item = orm.ExecuteProcedure("onmm3.get_items_by_name", "First Item", DbMapperParameter.RefCursor)
                .SingleOrDefault();
            Assert.AreEqual(1, item.Id);
            Assert.AreEqual("First Item", item.Name);
        }
        ...
        {
            var items = orm.ExecuteProcedure("onmm3.get_items_by_name", "non existing", DbMapperParameter.RefCursor);
        ...
        public void ExecuteProcedureWithoutRefCursor()
        {
            ...
            object result = orm.ExecuteProcedure("onmm3.rename_item", 1, newName);
            Assert.IsNull(result);
            Assert.AreEqual(newName, orm.QueryScalar<string>("select name from onmm3.item where id = :0", 1));
        }
        ...
        public void InsertWithImplicitConnection()
        {
            ...
            Assert.AreEqual(count + 2, orm.QueryScalar<int>("select count(1) from onmm3.item"));
        ...
        public void InsertAndSelectBigClob()
        {
            ...
            //insert
            dynamic item = new Entity("onmm3.bigclobtest");
            item.Text = largeString;
            orm.Insert(item);

            //select
            item = orm.Query("select * from onmm3.bigclobtest").SingleOrDefault();
            Assert.AreEqual(largeString, item.Text);

            //scalar
            string largeStringFetched = orm.QueryScalar<string>("select text from onmm3.bigclobtest");
        ...
        public void InsertBigClobWithNonQuery()
        {
            ...
            orm.NonQuery("insert into onmm3.bigclobtest (text) values (:0)", largeString);
        ...
        {
            dynamic item1 = new Entity("onmm3.item_composite_key", "id, type");
            item1.Id = 1;
            item1.Type = 1;
            orm.Insert(item1);

            dynamic item2 = new Entity("onmm3.item_composite_key", "id,type");
            item2.Id = 1;
            item2.Type = 2;
            orm.Insert(item2);

            orm.Delete(item1);
            var list = orm.Query("select * from onmm3.item_composite_key");
            Assert.AreEqual(2, list.Single().Type);

            orm.Delete(item2);
            Assert.AreEqual(0, orm.QueryScalar<int>("select count(1) from onmm3.item_composite_key"));
        }
        ...
        public void LoadStaticShouldWarn()
        {
            ...
            var item = orm.Query<ItemWrongDefinition>("select id, name, decimal_value, date_value from onmm3.item where id = :0", 1).SingleOrDefault();
        ...
        public void ClobTests(string testValue)
        {
            ...
            var fromDb = orm.QueryScalar<string>("select text from onmm3.bigclobtest");
